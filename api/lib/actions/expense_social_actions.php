...
function list_expense_comments_action(): void
{
    ...
    $stmt = $pdo->prepare(
        'SELECT c.id, c.user_id, u.nickname, c.body, c.created_at
...
function add_expense_comment_action(): void
{
    ...
    $pdo->prepare(
        'INSERT INTO ' . $commentsTable . ' (expense_id, trip_id, user_id, body)
         VALUES (:expense_id, :trip_id, :user_id, :body)'
    )->execute([
        'expense_id' => $expenseId,
        'trip_id'    => $tripId,
        'user_id'    => $userId,
