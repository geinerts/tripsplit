...
<head>
  ...
  <title>Confirm Email Change · Splyto</title>
  <style>
...
<div class="card">
  ...
  ?>
    <h1>Invalid link</h1>
    <p>This email change link is invalid or expired. Request a new one from the app.</p>
  <?php else: ?>
    <h1>Confirm new email</h1>
    <p>Click below to confirm your new account email.</p>

    <button id="btn">Confirm email change</button>
    ...
    <script>
      ...
      btn.addEventListener('click', async () => {
        ...
        try {
          const res = await fetch('/api/api.php?action=confirm_email_change', {
            method: 'POST',
          ...
          if (data.ok) {
            ...
            msg.textContent = 'Email changed successfully. You can continue using Splyto.';
            btn.style.display = 'none';
            return;
          }
          msg.className = 'msg error';
          msg.textContent = data.error || 'Could not confirm email change.';
          btn.disabled = false;
