...
<head>
  ...
  <title>Verify Email · Splyto</title>
  <script>
    (function () {
...
<div class="card">
  ...
  ?>
    <h1>Invalid link</h1>
    <p>This verification link is invalid or expired. Request a new one from the app.</p>
  <?php else: ?>
    <h1>Verify email</h1>
    <p>Click below to verify your email and activate your account.</p>

    <button id="btn">Verify email</button>
    ...
    <script>
      ...
      btn.addEventListener('click', async () => {
        ...
        try {
          const res = await fetch('/api/api.php?action=confirm_email_verification', {
            method: 'POST',
          ...
          if (data.ok) {
            ...
            msg.textContent = 'Email verified. You can now log in to Splyto.';
            btn.style.display = 'none';
            return;
          }
          msg.className = 'msg error';
          msg.textContent = data.error || 'Could not verify email.';
          btn.disabled = false;
