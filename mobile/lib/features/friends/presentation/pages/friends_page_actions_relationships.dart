...
extension _FriendsPageActionsRelationships on _FriendsPageState {
  ...
  Future<bool> _confirmRemoveFriend(FriendUser user) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(_txt(en: 'Remove friend', lv: 'Noņemt draugu')),
        content: Text(
          _txt(
            en: 'Remove ${user.nickname} from your friends?',
            lv: 'Noņemt ${user.nickname} no draugiem?',
          ),
        ),
        actions: [
          TextButton(
          ...
          FilledButton(
            onPressed: () => Navigator.of(context).pop(true),
            child: Text(_txt(en: 'Remove', lv: 'Noņemt')),
          ),
        ],
      ),
    );
  ...
  Future<void> _removeFriend(FriendUser user) async {
    if (_removeLoading.contains(user.id)) {
      return;
    }
    final confirmed = await _confirmRemoveFriend(user);
    if (!confirmed || !mounted) {
      return;
    }

    _updateState(() {
      _removeLoading.add(user.id);
    });

    try {
      await widget.controller.removeFriend(userId: user.id);
      if (!mounted) {
        return;
      }
      _showSnack(
        _txt(
          en: '${user.nickname} removed from friends.',
          lv: 'Lietotājs ${user.nickname} noņemts no draugiem.',
        ),
      );
      await _loadSnapshot(showLoader: false);
    } on ApiException catch (error) {
      ...
      _showSnack(
        _txt(en: 'Failed to remove friend.', lv: 'Neizdevās noņemt draugu.'),
        isError: true,
      );
    } finally {
      if (mounted) {
        _updateState(() {
          _removeLoading.remove(user.id);
        });
      }
    }
  }
